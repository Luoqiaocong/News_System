...
from Repo import UserHistoryRepo
from Schemas.UserHistorySchema import UserHistoryResponse
...
async def delete(
    ...
    count = await UserHistoryRepo.delete_history(user.id, news_ids, db)
...
async def get(
    ...
    rows,total =  await UserHistoryRepo.get_all_history(user.id, page, page_size, db)
    hist_lt = [
        {
            **news.__dict__,
            "viewed_at": viewed_at,
    ...
    data = UserHistoryResponse(hist_lt=hist_lt, total=total)
...
async def clear(
    ...
    count = await UserHistoryRepo.remove_all(user.id, db)

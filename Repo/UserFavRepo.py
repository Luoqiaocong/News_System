...
class UserFavRepo:
    ...
    async def add_favorite(self,news_id:int,user_id:int,db:AsyncSession):
        ...
        try:
            ...
            raise UserFavException(status_code=status.HTTP_409_CONFLICT,msg="您已收藏该新闻，请勿重新操作")


    async def remove_favorite(self,news_ids:list[int],user_id:int,db:AsyncSession):
        if not news_ids :
            return 0
    ...
    async def check_is_favorite(self,news_id:int,user_id:int,db:AsyncSession):
        # 仅查询 1，不拉取整行模型对象
        query = select(UserFavorite).filter_by(user_id=user_id, news_id=news_id).limit(1)
        res = await db.execute(query)
        return res.scalar() is not None

    async def get_all_favorites(self,user_id:int,page:int,page_size:int,db:AsyncSession):
